...
5.  **Analisi Statica e Igiene del Codice (Politica "Zero Diagnostic"):** Prima di qualsiasi commit o rilascio di nuove funzionalità, è obbligatorio eseguire l'analisi statica in tutti i contesti di progetto:
    ...
    *   **Formattazione Canonica:** Eseguire `dart format` su tutti i file modificati prima di ogni commit. La pipeline CI verifica la formattazione con `dart format --output=none --set-exit-if-changed`; qualsiasi deviazione dallo stile canonico del formatter causa il fallimento della build.
