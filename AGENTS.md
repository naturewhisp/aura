    ...
    *   **Criteri Specifici di Risoluzione (Flutter 3.22+):**
        ...
        *   **Ottimizzazioni Strutturali di Dart 3:**
            ...
            *   **Invarianti del Dominio di Provisioning e Lifecycle (Fase 6.3+):**
                ...
                *   **Immutabilità Tassativa e Validazione JSON-Safe delle Collezioni nei DTO:** Tutti i costruttori di DTO che accettano mappe o liste (anche via costruttore pubblico e `copyWith`) devono eseguire immediatamente la copia non modificabile (`List.unmodifiable(...)`, `Map.unmodifiable(...)`). Qualsiasi mappa di metadati o diagnostica deve essere validata ricorsivamente rifiutando chiavi non-stringa, valori numerici non finiti (`NaN`/`Infinity`) o tipi di oggetto non serializzabili JSON.
