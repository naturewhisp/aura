...
void main() {
  group('GameConfigLoader & ConfigSource Tests -', () {
    test('EmbeddedFallbackConfigSource loads embedded configuration', () {
      ...
      expect(embedded.loadStringSync('test_config.json'), equals('{"hello": "world"}'));
    });

    test('GameConfigLoader preloads and falls back to default', () {
      // Usa embedded source
      final embedded = EmbeddedFallbackConfigSource({
        'panopticon_identity.json': '{"identity_id": "test_identity", "display_name": "TEST"}'
      });
      
      GameConfigLoader.setSource(embedded);
      
      // Caricamento con path custom caricato dall'embedded
      final idDef = GameConfigLoader.loadIdentityDefinition('test', customPath: 'panopticon_identity.json');
      ...
      final fallbackIdDef = GameConfigLoader.loadIdentityDefinition('panopticon', customPath: 'unknown.json');
    ...
    test('preloadConfig caches async loads', () async {
      final source = EmbeddedFallbackConfigSource({
        'assets/config/panopticon_identity.json': '{"identity_id": "async_identity", "display_name": "ASYNC"}'
      });
      ...
      final idDef = GameConfigLoader.loadIdentityDefinition('async', customPath: 'assets/config/panopticon_identity.json');
