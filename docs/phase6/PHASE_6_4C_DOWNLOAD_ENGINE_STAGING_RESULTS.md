...
- Il resume è consentito **unicamente se** il checkpoint contiene un **ETag forte** che rispetta rigorosamente la sintassi `RegExp(r'^"[^"]+"$')` tramite il metodo condiviso `DownloadCheckpoint.isValidStrongEtag`. ETag deboli (`W/"..."`), nulli, privi di virgolette o con virgolette interne malformate (es. `"a"b"`) vengono tassativamente rifiutati.
...
### 1.5 Applicazione Effettiva del Timeout di Richiesta ed Inattività
...
### 1.6 Gestione dei Redirect Cross-Origin e Sicurezza
